...
#define SAFE_FREE_MULTI_GPU(X, Y)                                              \
  if ((X) != NULL) {                                                           \
    int current_dev = 0;                                                       \
...
#define SAFE_ALOC_MANAGED(X, Y)                                                \
  CUDA_SAFE_CALL(cudaMallocManaged((void **)&(X), (Y)));

// ================================== Print ====================================
// print 1D array
template <typename T> void print_1d_array(T *input, int length) {
...
// ================================== Correctness
// ====================================
template <typename T> bool check_equal(const T *x, const T *y, size_t m) {
  bool correct = true;
...
// ================================== Random
// ====================================
inline double rand0to1() { return ((double)rand() / (double)RAND_MAX); }
...
// ================================== Reduction
// ====================================
template <typename vT> __forceinline__ __device__ vT sum_32_shfl(vT sum) {
#pragma unroll
...
// reduce sum
template <typename T> T reduce_sum(T *input, int length) {
  if (length == 0)
    return 0;
  T sum = 0;
  for (int i = 0; i < length; i++)
...
IdxType csr_findRowIdxUsingNnzIdx(const IdxType *rowPtr, IdxType height,
                                  IdxType nnzIdx) {
