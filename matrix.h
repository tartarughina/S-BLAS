      ...
      if (policy == replicate) {
        for (unsigned i = 0; i < n_gpu; i++) {
          ...
          CUDA_SAFE_CALL(cudaMemcpy(cooColIdx_gpu[i], cooColIdx,
                                    get_nnz_idx_size(),
                                    cudaMemcpyHostToDevice));
          CUDA_SAFE_CALL(cudaMemcpy(cooVal_gpu[i], cooVal, get_nnz_val_size(),
                                    cudaMemcpyHostToDevice));
        }
      }
      if (policy == segment) {
        ...
        for (unsigned i = 0; i < n_gpu; i++) {
          ...
          CUDA_SAFE_CALL(cudaMemcpy(cooRowIdx_gpu[i], &cooRowIdx[i * avg_nnz],
                                    get_gpu_nnz_idx_size(i),
                                    cudaMemcpyHostToDevice));
          CUDA_SAFE_CALL(cudaMemcpy(cooColIdx_gpu[i], &cooColIdx[i * avg_nnz],
                                    get_gpu_nnz_idx_size(i),
                                    cudaMemcpyHostToDevice));
    ...
    if (n_gpu != 0 and policy != none) {
      ...
      if (policy == replicate) {
        ...
        for (unsigned i = 0; i < n_gpu; i++) {
          ...
          CUDA_SAFE_CALL(cudaMemcpy(csrRowPtr_gpu[i], fixedRowPtr,
                                    get_gpu_row_ptr_size(i),
                                    cudaMemcpyHostToDevice));
          CUDA_SAFE_CALL(cudaMemcpy(csrColIdx_gpu[i], &csrColIdx[i * avg_nnz],
                                    get_gpu_col_idx_size(i),
                                    cudaMemcpyHostToDevice));
          CUDA_SAFE_CALL(cudaMemcpy(csrVal_gpu[i], &csrVal[i * avg_nnz],
                                    get_gpu_nnz_val_size(i),
                                    cudaMemcpyHostToDevice));
  ...
  void sync2gpu(unsigned _n_gpu, enum GpuSharePolicy _policy) {
    ...
    if (policy == replicate) {
      ...
      for (unsigned i = 0; i < n_gpu; i++) {
        ...
        CUDA_SAFE_CALL(cudaMemcpy(val_gpu[i], val, get_mtx_size(),
                                  cudaMemcpyHostToDevice));
      }
    } else if (policy == segment) {
      SAFE_ALOC_HOST(val_gpu, n_gpu * sizeof(DataType *));
